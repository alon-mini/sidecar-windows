...
func (m *Model) runCheckPrerequisites() tea.Cmd {
	...
	return func() tea.Msg {
		...
		case version.InstallMethodScoop:
			if _, err := exec.LookPath("scoop"); err != nil {
				return UpdateErrorMsg{Step: "check", Err: fmt.Errorf("scoop not found in PATH")}
			}
...
func (m *Model) runInstallPhase() tea.Cmd {
	...
	return func() tea.Msg {
		...
		if sidecarUpdate != nil {
			...
			case version.InstallMethodScoop:
				cmd := exec.Command("scoop", "update", "sidecar")
				output, err := cmd.CombinedOutput()
				if err != nil {
					return UpdateErrorMsg{Step: "sidecar", Err: fmt.Errorf("%v: %s", err, output)}
				}
		...
		if tdUpdate != nil && tdUpdate.HasUpdate && tdUpdate.Installed {
			...
			case version.InstallMethodScoop:
				cmd := exec.Command("scoop", "update", "td")
				if output, err := cmd.CombinedOutput(); err != nil {
					return UpdateErrorMsg{Step: "td", Err: fmt.Errorf("%v: %s", err, output)}
				}
...
Add my code projects to ` + config.ConfigPath() + ` using this format:
