...
// On Unix: export KEY='val' / unset KEY
// On Windows: $env:KEY = "val" / Remove-Item Env:\KEY
func GenerateExportCommands(overrides map[string]string) []string {
	var cmds []string
	for key, value := range overrides {
		if value == "" {
			cmds = append(cmds, unsetEnvCmd(key))
		} else {
			cmds = append(cmds, setEnvCmd(key, value))
		}
	}
	return cmds
}
...
// Platform-aware: uses export/unset on Unix, $env:/Remove-Item on Windows.
func GenerateSingleEnvCommand(overrides map[string]string) string {
	var parts []string
	for key, value := range overrides {
		if value == "" {
			parts = append(parts, unsetEnvCmd(key))
		} else {
			parts = append(parts, setEnvCmd(key, value))
		}
	}
