...
const (
	// configDirName is the application-specific subdirectory inside the OS config root.
	configDirName = "sidecar"
...
// If path is empty, uses the OS-standard config directory:
//   - Windows: %APPDATA%\sidecar\config.json
//   - macOS:   ~/Library/Application Support/sidecar/config.json
//   - Linux:   ~/.config/sidecar/config.json
...
// Handles both ~/path (Unix) and ~\path (Windows).
...
// configBaseDir returns the OS-standard config directory for sidecar.
...
func configBaseDir() string {
	...
	return filepath.Join(configDir, configDirName)
}
...
func ConfigPath() string {
	...
	return filepath.Join(base, configFile)
}

// ConfigDir returns the config directory path (for display in UI).
