...
```powershell
scoop bucket add sidecar-windows https://github.com/alon-mini/sidecar-windows
scoop install sidecar-windows/sidecar
```

This installs `sidecar.exe`, `td.exe`, and `td-sync.exe`.

**Manual download:** Grab the latest `_windows_amd64.zip` from [Releases](https://github.com/alon-mini/sidecar-windows/releases), extract, and add the folder to your `PATH`.

**Build from source:**

```powershell
git clone https://github.com/alon-mini/sidecar-windows.git
cd sidecar-windows
$env:CGO_ENABLED=0; go build -o bin/sidecar.exe ./cmd/sidecar
$env:CGO_ENABLED=0; go build -o bin/td.exe ./td-fork
$env:CGO_ENABLED=0; go build -o bin/td-sync.exe ./td-fork/cmd/td-sync
```
...
**More options:** [Binary downloads](https://github.com/alon-mini/sidecar-windows/releases) · [Manual install](docs/getting-started.md)
